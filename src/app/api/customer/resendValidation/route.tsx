...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!isValidPassword) {
      return NextResponse.json({
        message: 'Invalid password. Please enter a valid password or reset it.',
        status: 400,
        success: false,
      });
    }
    ...
    if (customer.isVerified) {
      return NextResponse.json({
        message:
          'Your account is already verified. Please log in to access the dashboard.',
        status: 200,
        success: true,
      });
    }
    // Convert verifyTokenExpiry to Date if it is not already
    const verifyTokenExpiry = customer.verifyTokenExpiry
      ? new Date(customer.verifyTokenExpiry)
      : null;
    ...
    if (!isExpired) {
      return NextResponse.json({
        message: 'Your previous verification token is not expired yet.',
        status: 400,
        success: false,
      });
    }
